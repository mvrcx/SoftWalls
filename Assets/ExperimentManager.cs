...
public class ExperimentManager : MonoBehaviour
{
    ...
    [Header("Participant")] public string participantId;
    [Header("XR")] public Transform xrOrigin;
    [Header("Rooms")] public RoomController testingRoom;
    public List<RoomController> experimentalRooms;
    [Header("Logger")] public ObjectiveMetricsLogger logger;
    private List<RoomController> randomizedRooms;
    private int currentIndex = -1;
    public bool experimentFinished { get; private set; } = false;
    [Header("UI")] public GameObject stopButton;
    public GameObject nextRoomButton;
    public TextMeshProUGUI roomNameText;
    [Header("End Message")] public Canvas endMessageCanvas;
    ...
    void Start()
    {
        ...
        // LOGGING: Start the testing room
        if (CSVDataManager.Instance != null)
            CSVDataManager.Instance.StartRoomTracking(testingRoom.roomName);
        ...
        if (endMessageCanvas != null) endMessageCanvas.gameObject.SetActive(false);
    }
    ...
    void EnterTestingRoom() { testingRoom.Activate(xrOrigin); }

    public void GoToNextRoom()
    {
        if (experimentFinished) return;

        // LOGGING: Finalize the room the player is LEAVING
        if (CSVDataManager.Instance != null)
            CSVDataManager.Instance.EndRoomTracking();

        if (currentIndex >= 0 && currentIndex < randomizedRooms.Count)
            logger?.StopRecordingAndSave();

        if (currentIndex == -1) testingRoom.Deactivate();
        else if (currentIndex < randomizedRooms.Count) randomizedRooms[currentIndex].Deactivate();
        ...
        if (currentIndex < randomizedRooms.Count) {
            ...
            // LOGGING: Start the new randomized room
            if (CSVDataManager.Instance != null)
                CSVDataManager.Instance.StartRoomTracking(room.roomName);

            // ... UI and logging logic continues ...
            ...
            // LOGGING: Save final data
            if (CSVDataManager.Instance != null) CSVDataManager.Instance.SaveToCSV();
    ...
    public void StopRoom() {
        if (experimentFinished || currentIndex < 0 || currentIndex >= randomizedRooms.Count) return;
        logger?.StopRecordingAndSave();
        stopButton.SetActive(false);
